...
TEST(DateTest, FromComponentsInvalidDay) {
  EXPECT_THROW(Date::from_components(2024, 4, 31),
...
TEST(PaymentMethodTest, ToString) {
  ...
  EXPECT_STREQ(to_string(PaymentMethod::ACCOUNT_RECEIVABLE),
                "Account Receivable");
  EXPECT_STREQ(to_string(PaymentMethod::CREDIT_CARD), "Credit Card");
  EXPECT_STREQ(to_string(PaymentMethod::POST_DATED_CHECK),
                "Post-Dated Check");
